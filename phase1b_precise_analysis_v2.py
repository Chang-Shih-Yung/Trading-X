...
class Phase1BPreciseAnalyzer:
    ...
    def analyze_data_usage_consistency(self, json_spec: Dict, py_content: str) -> Dict[str, Any]:
        ...
        # 正確訪問 JSON 結構
        strategy_graph = json_spec.get('strategy_dependency_graph', {})
        signal_output = strategy_graph.get('signal_output_format', {})
        
        # JSON 數據類型規範（從輸出格式提取）
        if 'enhanced_volatility_adapted_signal' in signal_output:
            signal_format = signal_output['enhanced_volatility_adapted_signal']
            analysis['json_data_types']['signal_output'] = {
                'fields': list(signal_format.keys()),
                'required': ['signal_type', 'signal_strength', 'signal_confidence', 'timestamp'],
                'validation': signal_format
            }
        
        # 從 computation_flow 提取數據結構
        computation_flow = strategy_graph.get('computation_flow', {})
        for layer_name, layer_config in computation_flow.items():
            operations = layer_config.get('operations', {})
            for op_name, op_config in operations.items():
                if 'output' in op_config:
                    output_name = op_config['output']
                    analysis['json_data_types'][output_name] = {
                        'fields': [output_name],
        ...
        # Python 數據類型實現 - 簡化版本
        dataclass_names = ['VolatilityMetrics', 'AdaptiveSignalAdjustment', 'SignalContinuityMetrics']
        
        for class_name in dataclass_names:
            if class_name in py_content:
                # 簡單檢測數據類存在
        ...
        # DataClass 對齊分析
        # 檢查關鍵數據結構
        key_structures = ['VolatilityMetrics', 'AdaptiveSignalAdjustment']
        
        for struct_name in key_structures:
            if struct_name in analysis['python_data_types']:
                # 根據結構名稱檢查對應的 JSON 要求
                if struct_name == 'VolatilityMetrics':
                    # 檢查是否有波動性相關欄位
                    required_patterns = [
                        'current_volatility', 'volatility_trend', 'volatility_percentile',
                        'regime_stability', 'timestamp'
                    ]
                    
                elif struct_name == 'AdaptiveSignalAdjustment':
                    # 檢查信號調整要求
                    required_patterns = [
                        'original_signal', 'adjusted_signal', 'adjustment_factor',
                        'adjustment_reason', 'confidence_boost'
                    ]
                else:
                    required_patterns = []
                
                if required_patterns:
                    found_patterns = sum(1 for pattern in required_patterns if pattern in py_content)
                    coverage = found_patterns / len(required_patterns)
                    
                    analysis['dataclass_alignment'][struct_name] = {
        ...
        # 計算類型一致性
        if analysis['dataclass_alignment']:
            total_coverage = sum(alignment['field_coverage'] 
                               for alignment in analysis['dataclass_alignment'].values())
            analysis['type_consistency'] = total_coverage / len(analysis['dataclass_alignment'])
        else:
            analysis['type_consistency'] = 0.0
        
        print(f"   JSON 數據類型: {len(analysis['json_data_types'])}")
        print(f"   Python 數據類型: {len(analysis['python_data_types'])}")
        print(f"   關鍵結構對齊: {len(analysis['dataclass_alignment'])}")
        print(f"   類型一致性: {analysis['type_consistency']:.1%}")
        
        for struct_name, alignment in analysis['dataclass_alignment'].items():
            coverage = alignment['field_coverage']
            found = alignment['found_patterns']
            total = alignment['total_patterns']
            print(f"   - {struct_name}: 覆蓋率 {coverage:.1%} ({found}/{total})")
    ...
    def analyze_integration_completeness(self, json_spec: Dict, py_content: str) -> Dict[str, Any]:
        ...
        # JSON 外部依賴
        strategy_graph = json_spec.get('strategy_dependency_graph', {})
        dependencies = strategy_graph.get('dependencies', {})
        external_libs = dependencies.get('external_libraries', [])
        analysis['external_dependencies']['json'] = external_libs
