...
class ExternalMarketAPIs:
    ...
    async def get_binance_realtime_data(self, symbol: str) -> Optional[BinanceMarketData]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ 即時API失敗，切換備用數據源: {api_error}")
            
            # 🔄 備用：內部數據服務
            backup_data = await self._get_binance_backup_data(symbol)
            if backup_data:
                logger.info(f"🔄 使用備用數據 {symbol}: ${backup_data.current_price:.4f}")
                return backup_data
    ...
    async def _get_binance_backup_data(self, symbol: str) -> Optional[BinanceMarketData]:
        """備用數據獲取方法"""
        try:
            # 使用內部市場數據服務
            ...
            logger.error(f"備用數據獲取失敗 {symbol}: {e}")
            return None
    ...
    async def get_fear_greed_analysis(self) -> FearGreedAnalysis:
        ...
        return FearGreedAnalysis(
            ...
            weight_in_decision=0.10,  # 默認10%權重
            market_interpretation="市場情緒中性，無明顯偏向",
            timestamp=now
        )
