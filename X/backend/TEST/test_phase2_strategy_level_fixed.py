...
class Phase2StrategyLevelTest:
    ...
    async def _simulate_multi_timeframe_analysis(self, timeframe_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        signal_consistency = len(set(all_signals)) / len(all_signals) if all_signals else 0
        ...
        return {
            ...
            "consensus_accuracy": consensus_accuracy,
            "signal_consistency": 1.0 - signal_consistency,  # 反向計算，一致性越高越好
    ...
    async def _simulate_risk_management(self, portfolio_exposure: float, position_size: float, stop_loss_distance: float, expected_action: str, expected_risk_level: str) -> Dict[str, Any]:
        ...
        # 計算風險分數 
        risk_score = (portfolio_exposure * 0.4 + position_size * 0.3 + stop_loss_distance * 0.3)
    ...
    async def _simulate_signal_priority_sorting(self, signals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for signal in signals:
            ...
            time_weight = {
                ...
                "MEDIUM": 0.6,
                "LOW": 0.4
            }.get(signal.get("time_sensitivity", "MEDIUM"), 0.6)
            
            # 計算優先級分數
            priority_score = (
                confidence * 0.3 +
                profit_potential * 10 * 0.25 +  # 標準化
                min(risk_reward / 10, 1.0) * 0.25 +
                time_weight * 0.2
            )
    ...
    def _evaluate_ranking_accuracy(self, sorted_signals: List[Dict[str, Any]], original_signals: List[Dict[str, Any]]) -> float:
        """評估排序準確性"""
        # 簡化評估：檢查前兩個信號是否包含最高優先級的信號
        top_2_signals = sorted_signals[:2]
        top_2_ids = [s["signal_id"] for s in top_2_signals]
        
        # 查找原始信號中應該排在前面的信號（CRITICAL時間敏感性或高置信度）
        priority_signals = [
            s for s in original_signals 
            if s.get("time_sensitivity") == "CRITICAL" or s.get("confidence", 0) > 0.9
        ]
        ...
        # 計算前兩個位置中包含高優先級信號的比例
        matches = len(set(top_2_ids) & set(priority_ids))
        accuracy = matches / min(2, len(priority_ids))
    ...
    async def _simulate_strategy_execution_optimization(self, strategy_count: int, signal_volume: int, target_throughput: int) -> Dict[str, Any]:
        ...
        # 計算實際吞吐量
        optimization_factor = 1.0 + parallelization_boost + batch_efficiency
        actual_throughput = base_throughput * optimization_factor
        
        # 添加一些隨機變化
        actual_throughput *= (0.9 + np.random.uniform(0, 0.2))
